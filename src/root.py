...
class Root(cust.CTk):
    ...
    def __init__(self, **kwargs):
        ...
        self.URL_copy = cust.CTkButton(self.URL_frame, text='Paste', height=5, fg_color=self.URL_entryField._fg_color, bg_color='transparent', border_spacing=8, border_color=self.URL_entryField._border_color,
                                       border_width=2, width=8, corner_radius=self.URL_entryField._corner_radius, font=cust.CTkFont(family='Helvatica', size=13, weight='bold'), command=self.copy_URL)
    ...
    def show_info(self):
        ...
        self.vid_length = cust.CTkLabel(self.details_frame, font=cust.CTkFont(
            'Tajawal', weight='normal', size=17), text=f'video duration: {video_length}', justify='left')
        ...
        self.vid_views = cust.CTkLabel(self.details_frame, font=cust.CTkFont(
            'Tajawal', weight='normal', size=17), text=f'{video_views} view', justify='left')
    ...
    def dl_thumbnail(self) :
        try:
            video_thumbnail.save(f"{Images.current_path}/Downloads/{video_title}.png")
            messagebox.showinfo(message=f'Thumbnail downloaded in "{Images.current_path}/Downloads"')
        except Exception as e:
            messagebox.showinfo(message=f'Download failed')
    def url_func(self):
